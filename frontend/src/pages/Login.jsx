  ...
  return (
    <div className="min-h-[70vh] flex items-center justify-center px-4">
      <form onSubmit={onSubmit} className="w-full max-w-md rounded-xl border bg-white p-6 shadow">
        ...
        <p className="mt-1 text-sm text-slate-600">Use username or email.</p>
        {error && <div className="mt-3 rounded bg-rose-50 p-2 text-rose-700 border border-rose-200">{error}</div>}
        <div className="mt-4">
          <label className="text-sm text-slate-700">Username or Email</label>
          <input className="mt-1 w-full rounded-md border px-3 py-2" value={form.identifier} onChange={e=>setForm(f=>({...f, identifier:e.target.value}))} required />
        </div>
        <div className="mt-3">
        ...
        <div className="mt-3 text-sm text-slate-600">
          No account? <Link to="/signup" className="text-sky-700">Sign up</Link>
        </div>
      </form>
    </div>
  )
